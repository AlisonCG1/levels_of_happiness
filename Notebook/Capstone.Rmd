...
```{r}
# Calculate unhappiness by grouping the dataset by Country and Year
unhappiness <- Happiness_levels %>%
  filter(Feeling_of_happiness %in% c(3, 4)) %>%
  group_by(Country, Year) %>%
  summarise(Unhappiness_Count = n(), .groups = "drop")

# Print the unhappiness dataframe
print(unhappiness)
```
```{r}
unhappiness <- drop_na(drop_na(unhappiness))
```
...
```{r}
lmAge <- Happiness_levels%>%
  lm(Feeling_of_happiness~Age, data = .)
  summary(lmAge)
```
```{r}
lmSex <- Happiness_levels%>%
  lm(Feeling_of_happiness~Sex, data = .)
  summary(lmSex)
```
```{r}
lm_sex_age <- population_happiness%>%
  lm(Feeling_of_happiness ~ Sex + Age + CountryCode, data = .)
  summary(lm_sex_age)
```

```{r}
unique(population_happiness$CountryCode)%>%
  sort()
```
...
```{r}
ggplot(Happiness_levels, aes(Age, Feeling_of_happiness)) +
  geom_point()
```
