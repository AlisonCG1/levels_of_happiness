...
```{r}
#mean_data <- merge(mean_copy, region, by = "Country", by.y = CountryCode, all.x = TRUE)
...
```{r}
mean_copy <- merge(mean_copy, region, by = "Country", all = TRUE)
```
...
```{r}
plot_2 <- plot_2[, !colnames(plot_2) %in% c("CountryCode")]
plot_2 <- plot_2[, !colnames(plot_2) %in% c("Year")]

# Increase the cardinality_threshold to 2000
plot_2 <- plot_2 %>% ggpairs(cardinality_threshold = 2000, title = "correlogram with ggpairs()")
```
