...
```{r}
countries <- unique(filtered_happiness$Country)
...
```{r}
mean_data <- population_happiness %>%
  group_by(Country) %>%
...
mean_data <- drop_na(mean_data, Country)

mean_data
```

```{r}
mean_data <- left_join(mean_data, region, by= "Country")
