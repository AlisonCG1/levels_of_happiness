...
```{r}
population_happiness <- merge(Population_total, filtered_happiness, by.x = c("CountryCode", "Year"), by.y = c("Country", "Year"))

population_happiness
```

```{r}
# Delete the column using the subset() function
population_happiness <- subset(population_happiness, select = -CountryCode.y)
...
any_duplicate <- anyDuplicated(population_happiness)
...
```{r}
# Drop duplicates based on all columns
population_happiness <- population_happiness[!duplicated(population_happiness), ]
...
```{r}
# Convert 'CountryCode' column to character in 'filtered_happiness' dataframe
filtered_happiness$CountryCode <- as.character(filtered_happiness$CountryCode)

# Perform the left join
merged_table_test <- merge(filtered_happiness, Population_total, by = c("CountryCode", "Year"))
...
merged_table <-  dplyr::left_join(filtered_happiness, Population_total, by.y = c("CountryCode", "Year"), by.x = c("Country", "Year"))
